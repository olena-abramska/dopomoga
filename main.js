...
aromatherapy.addEventListener('click', function() {
    ...
    first_word.appendChild(first_string);
    first_string.classList.add('first-string');
    first_string.innerText = " - це приємна процедура, дихання ефірними оліями,";
    let first_paragraph = document.createElement('div');
    first_string.appendChild(first_paragraph);
    first_paragraph.classList.add('first-paragraph');
    first_paragraph.innerText = "фітонцидами квіток та інших ароматичних речовин, яка використовується для укріплення фізичного та психологічного здоров’я людини. Також ароматерапія дуже корисна для долгляду за тілом. Зазвичай, сеанс ароматерапії триває приблизно 20 хвилин. Для кращого ефекту під час сеансу вмикають приємну розслаблюючу музику, яка стимулює розслаблення та підвищує дію ароматерапії. Після цієї процедури людина заспокоюється, покращується настрій. Ароматерамія може принести полегшення людям, які страждають затяжними захворюваннями. Також вона може полегшити побічні ефекти від деяких ліків. Різні аромати можуть по-різному впливати на нервову систему. Хоча ароматерапія дуже корисна від стресу та гарно розслабляє, ви повинні розуміти, що вона не є основними ліками. Тому якщо ви маєте серйозну хворобу, вам потрібно звернутися до лікаря, адже ароматерапія лише пом’якшує стан, але не лікує."
